...
int main(int argc, char* argv[]) {
    ...
    #ifdef __ANDROID__
    __android_log_print(ANDROID_LOG_INFO, "Agonia", "Initializing Firebase...");
#else
    ...
    app = firebase::App::Create(options, env, activity);
#else
    // 2b. DESKTOP: Standard initialization
    app = firebase::App::Create(options);
#endif

    if (!app) {
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_ERROR, "Agonia", "ERROR: Failed to initialize Firebase App!");
#else
